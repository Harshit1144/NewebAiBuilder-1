          ...
          <div>
              ...
              {features.map((feature, index) => (
                <div 
                  ...
                  className={`flex items-start p-4 rounded-lg transition-all duration-300 transform cursor-pointer ${
                    activeFeature === index 
                      ? `bg-${feature.color}-50 border border-${feature.color}-100 scale-105 shadow-md` 
                      : "hover:bg-gray-50 border border-transparent hover:shadow"
                  }`}
                  onClick={() => setActiveFeature(activeFeature === index ? null : index)}
                >
                  <div className={`flex-shrink-0 w-12 h-12 bg-${feature.color === 'blue' ? 'primary/10' : `${feature.color}-100`} rounded-full flex items-center justify-center mr-5 transition-all duration-300 ${activeFeature === index ? `bg-${feature.color === 'blue' ? 'primary' : `${feature.color}-500`} shadow-lg` : ''}`}>
                    <div className={`transition-all duration-300 ${activeFeature === index ? 'text-white' : feature.color === 'blue' ? 'text-primary' : `text-${feature.color}-600`}`}>
                      {feature.icon}
                    </div>
                  </div>
                  <div className="flex-1">
                    <h3 className="text-lg font-semibold mb-1 flex items-center">
                      {feature.title}
                      {activeFeature === index && (
                        <span className="ml-2 text-xs bg-primary text-white px-2 py-0.5 rounded-full">Click for details</span>
                      )}
                    </h3>
            ...
            <div className="mt-8">
              ...
              >
                ...
                <span className="absolute inset-0 bg-gradient-to-r from-blue-600 to-primary transform scale-x-0 group-hover:scale-x-100 origin-left transition-transform duration-300"></span>
              </Button>
            </div>
          </div>
